...
export default function MainPage() {
  ...
  const [languageID, setLanguageID] = useState<number>(() => {
    ...
    return storedLanguageID ? Number.parseInt(storedLanguageID, 10) : LanguageId.Python3;
  });
  const [prevLanguageID, setPrevLanguageID] = useState<number>(LanguageId.Python3);
        ...
        <Panel defaultSize={70} minSize={30}>
          {renderFirst === RenderFirst.WelcomeMarkdown && (
            ...
            >
              ...
              >
                <Markdown remarkPlugins={[[remarkGfm, { singleTilde: false }], [remarkToc, { tight: true, maxDepth: 5, heading: 'contents' }]]} rehypePlugins={[[rehypeSlug]]}>{readme}</Markdown>
              </div>
            </div>
          )}
